//! Halal OS Desktop - Hijri Lunar Calendar Converter
...
pub fn hijri_month_name(month: u32, lang: &str) -> &'static str {
    let idx = if month >= 1 && month <= 12 { (month - 1) as usize } else { 0 };
    if lang == "ar" {
        HIJRI_MONTHS_AR[idx]
    } else {
        HIJRI_MONTHS_EN[idx]
